        ...
        if self.current_task > 0:
            ...
            if type(dataset.train_loader.dataset.data) == torch.Tensor:
                ...
                try:
                    ...
                    # dataset.train_loader.dataset.data = dataset.train_loader.dataset.data.unsqueeze(1)
                    print(len(dataset.train_loader.dataset.data))
                    print(dataset.train_loader.dataset.data[0].shape)
                    result = torch.stack([(denorm(self.buffer.examples[i] * 255).type(torch.uint8).cpu()) for i in range(self.buffer.num_seen_examples)])
                    print(result.size())
