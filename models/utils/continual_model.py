...
from models.utils.onedrive_fake import download as dn_fake

def get_ckpt_remote_url(args):
    if args.pre_dataset == "cifar100":
        args.load_cp = 'models/pre_model/rs18_cifar100_new.pth'

    elif args.pre_dataset == "tinyimgR":
        args.load_cp = 'models/pre_model/pre_tiny_fixed.pth'

    elif args.pre_dataset == "imagenet":
        args.load_cp = 'models/pre_model/rs50_imagenet_full.pth'

    elif args.pre_dataset == "imagenetA":
        args.load_cp = 'models/pre_model/pretrain_on_A_best_model_r50_67_16.pth'

    elif args.pre_dataset == "imagenetB":
        args.load_cp = 'models/pre_model/pretrain_on_B_best_model_r50.pth.pth'

    elif args.pre_dataset == None:
        args.load_cp = '/home/tiansongsong/data/model/Rs18_random_initialization.pth'
...
class ContinualModel(nn.Module):
    ...
    def get_aux_dataset(self):
        ...
        elif self.args.pre_dataset == 'imagenet' or self.args.pre_dataset == 'imagenetA' or self.args.pre_dataset == 'imagenetB':
            aux_dset = None
            aux_test_dset = None
        else:
    ...
    def load_aux_dataset(self):
        ...
        if self.args.pre_dataset == 'imagenet' or self.args.pre_dataset == 'imagenetA' or self.args.pre_dataset == 'imagenetB':
            self.aux_dl = None
    ...
    def load_initial_checkpoint(self, ignore_classifier=False):
        ...
        #             print(f"Downloading checkpoint file :{self.args.load_cp}")
                    # print(f"Downloaded in: {self.args.load_cp}")
                # self.load_cp(self.args.load_cp, moco=True, ignore_classifier=ignore_classifier)
                # print("Loaded!")
    ...
    def load_cp(self, cp_path, new_classes=None, moco=False, ignore_classifier=False) -> None:
        ...
        if 'state_dict' in s:  # loading moco checkpoint x
            if not moco:
                raise Exception(
        ...
        if not ignore_classifier: # t
            if new_classes is not None: # x
                self.net.classifier = torch.nn.Linear(
                    self.net.classifier.in_features, self.num_aux_classes).to(self.device)
                for k in list(s):
                    if 'classifier' in k:
