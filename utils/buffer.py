...
class Buffer:
    ...
    def add_data(self, examples, labels=None, logits=None, task_labels=None, attention_maps=None):
        ...
        :param examples: tensor containing the images 未增强的输入图像
        :param labels: tensor containing the labels 标签
        :param logits: tensor containing the outputs of the network 本模型输出 预训练模型输出 #batch_size*(当前任务的长度+预训练任务的长度)
        :param task_labels: tensor containing the task labels
        :return:
        """
    ...
    def get_data(self, size: int, transform: transforms=None, return_index=False, to_device=None) -> Tuple:
        ...
        :param size: the number of requested items 即minibatch_size
