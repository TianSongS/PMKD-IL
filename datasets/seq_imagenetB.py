...
class SequentialIMAGENETB(ContinualDataset):
    ...
    def get_transform():
        transform = transforms.Compose(
            [transforms.ToPILImage(), SequentialIMAGENETB.TRANSFORM])
        return transform
    
    @staticmethod
    def get_transform_my():
        transform = transforms.Compose(
            [SequentialIMAGENETB.TRANSFORM])
        return transform

    @staticmethod
    def get_backbone():
        return resnet50(SequentialIMAGENETB.N_CLASSES_PER_TASK
                        * SequentialIMAGENETB.N_TASKS) # 返回10分类的模型
    ...
    def get_normalization_transform():
        transform = transforms.Normalize(SequentialIMAGENETB.MEAN,
                                         SequentialIMAGENETB.STD)
        return transform

    @staticmethod
    def get_denormalization_transform():
        transform = DeNormalize(SequentialIMAGENETB.MEAN,
                                SequentialIMAGENETB.STD)
        return transform
