...
def store_masked_loaders(train_dataset: datasets, test_dataset: datasets,
    ...
    if not hasattr(train_dataset, 'lazy_load') or not train_dataset.lazy_load:
        train_dataset.data = train_dataset.data[train_mask] # 只有第一个任务的数据
    if not hasattr(test_dataset, 'lazy_load') or not test_dataset.lazy_load:
        test_dataset.data = test_dataset.data[test_mask]

    train_dataset.targets = np.array(train_dataset.targets)[train_mask] #只有第一个任务的数据标签
    ...
    setting.test_loaders.append(test_loader) # 训练集迭代tasks次，存储目前所有的
    setting.train_loader = train_loader # 只存储当前的

    setting.i += setting.N_CLASSES_PER_TASK # setting是传入原数据的对象，同时又返回了两个数据集
    return train_loader, test_loader




def store_masked_loaders_imagenet(train_dataset: datasets.ImageFolder, test_dataset: datasets.ImageFolder,
                         setting: ContinualDataset, class_order=None) -> Tuple[DataLoader, DataLoader]:
    """
    Divides the dataset into tasks.
    :param train_dataset: train dataset
    ...
    if not hasattr(train_dataset, 'lazy_load') or not train_dataset.lazy_load:
        train_dataset.data = [train_dataset.data[i] for i in range(len(train_dataset.data)) if train_mask[i]]
    if not hasattr(test_dataset, 'lazy_load') or not test_dataset.lazy_load:
        test_dataset.data = [test_dataset.data[i] for i in range(len(test_dataset.data)) if test_mask[i]]
